...
func textChunk(text string) io.Reader {
	...
	// CRC計算とバッファへの書き込みを同時に行う MultiWriter
...
func main() {
	file, err := os.Open("../../input/PNG_transparency_demonstration_1.png")
	if err != nil {
		panic(err)
	}
	defer file.Close()
	newFile, err := os.Create("PNG_transparency_demonstration_1.png")
	if err != nil {
		panic(err)
	}
	defer newFile.Close()
